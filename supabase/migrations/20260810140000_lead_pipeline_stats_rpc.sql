-- Lead pipeline / overview aggregates — avoid PostgREST max-rows cap on client counts.
-- SECURITY INVOKER so existing leads RLS (team + shared folders) still applies.

CREATE OR REPLACE FUNCTION public.lead_call_bucket(p_call_status text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN lower(trim(both FROM replace(coalesce(nullif(trim(both FROM coalesce(p_call_status, '')), ''), 'Not called'), '_', ' ')))
         IN ('', 'not called')
      THEN 'not_called'
    WHEN lower(trim(both FROM replace(coalesce(p_call_status, ''), '_', ' '))) = 'callback requested'
      THEN 'callback'
    WHEN lower(trim(both FROM replace(coalesce(p_call_status, ''), '_', ' ')))
         IN ('answered', 'interested', 'meeting booked')
      THEN 'connected'
    WHEN lower(trim(both FROM replace(coalesce(p_call_status, ''), '_', ' ')))
         IN ('wrong number', 'not interested')
      THEN 'closed'
    ELSE 'attempted'
  END;
$$;

COMMENT ON FUNCTION public.lead_call_bucket(text) IS
  'Maps leads.call_status to dashboard call-pipeline bucket ids (mirrors src/lib/dashboardMetrics.js).';
...
COMMENT ON FUNCTION public.get_lead_pipeline_stats IS
  'Exact aggregated lead pipeline/overview stats (bypasses PostgREST max-rows). Filtered by RLS.';

GRANT EXECUTE ON FUNCTION public.lead_call_bucket(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_lead_pipeline_stats(
